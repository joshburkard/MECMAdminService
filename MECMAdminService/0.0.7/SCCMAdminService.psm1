<#
    Generated at 02/11/2026 20:35:53 by Josua Burkard
#>
...
function Get-CMASCollectionDirectMembershipRule {
    ...
    param(
        ...
        [Parameter(Mandatory=$false, ParameterSetName='ByNameAndName')]
        [Parameter(Mandatory=$false, ParameterSetName='ByIdAndName')]
        [Parameter(Mandatory=$false, ParameterSetName='ByValueAndName')]
        ...
        [Parameter(Mandatory=$false, ParameterSetName='ByNameAndId')]
        [Parameter(Mandatory=$false, ParameterSetName='ByIdAndId')]
        [Parameter(Mandatory=$false, ParameterSetName='ByValueAndId')]
        [Parameter(Mandatory=$false, ParameterSetName='ByName')]
        [Parameter(Mandatory=$false, ParameterSetName='ById')]
        [Parameter(Mandatory=$false, ParameterSetName='ByValue')]
        [long]$ResourceId
    ...
    process {
        try {
            ...
            # Build the API path
            $path = "wmi/SMS_CollectionRuleDirect"
            ...
            if ($ResourceName) {
                # Convert PowerShell wildcard to OData filter
                if ($ResourceName -match '\*') {
                    # Replace * with % for OData contains/startswith/endswith
                    $odataPattern = $ResourceName -replace '\*', ''
                    if ($ResourceName.StartsWith('*') -and $ResourceName.EndsWith('*')) {
                        # Contains
                        $filters += "contains(RuleName, '$odataPattern')"
                    } elseif ($ResourceName.StartsWith('*')) {
                        # EndsWith
                        $filters += "endswith(RuleName, '$odataPattern')"
                    } elseif ($ResourceName.EndsWith('*')) {
                        # StartsWith
                        $filters += "startswith(RuleName, '$odataPattern')"
                    }
                } else {
                    # Exact match
                    $filters += "RuleName eq '$ResourceName'"
                }
            }
            ...
            Write-Verbose "Fetching collection direct membership rules from Admin Service with path: $path"
            $res = Invoke-CMASApi -Path $path

            # Format results - exclude WMI metadata and OData metadata
            if ($res.value) {
                $rules = $res.value | Select-Object * -ExcludeProperty __*, @odata*
                return $rules
            } else {
                # No rules found
                return $null
